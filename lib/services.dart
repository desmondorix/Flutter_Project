...
abstract class Services {
  ...
  static Future<Map?> sendScore({required String username, required int score}) async {
    if (username.isNotEmpty) {
      String uri = "http://10.0.2.2/study_flutter/save_score.php";
      try {
        var res = await http.post(Uri.parse(uri), body: {
          "username": username,
          ...
          if (responseBody.isNotEmpty) {
            ...
            if (response is Map) {
              if (response["success"] == true) {
                ...
                // Login unsuccessful, print error message
                print("Sending failed: ${response['message']}");
                return null;
              }
            } else {
              print("Invalid server response format: $responseBody");
